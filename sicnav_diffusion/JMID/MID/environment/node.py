...
class Node(object):
    def __init__(
        ...
        self._aux_data = aux_data  # dictionary
    ...
    def get(self, tr_scene, state, padding=np.nan) -> np.ndarray:
        ...
        :param tr_scene: The timestep range (inclusive).
        ...
        padded_data_array[
            paddingl : max(length - paddingu, 0)
        ] = data_array  # If length - paddingu < 0, then paddingl will be 0 based on self.scene_ts_to_node_ts()
        return padded_data_array

    def get_mean_x_and_y(self):
        return np.array([self._aux_data["pos_x_mean"], self._aux_data["pos_y_mean"]])

    def get_x_and_y(self, scene_tr):
        """
        Returns the x and y positions with their means added back in.

        :param tr: The timestep range (inclusive).
        """
        tr = np.array(scene_tr) - self.first_timestep
        data_array = self.data[
            tr[0] : tr[1] + 1, [("position", "x"), ("position", "y")]
        ]
        return data_array

    def get_un_mean_centered_x_and_y(self, scene_tr):
        """
        Returns the x and y positions with their means added back in.
        ...
        data_array[:, 0] = data_array[:, 0] + self._aux_data["pos_x_mean"]
        data_array[:, 1] = data_array[:, 1] + self._aux_data["pos_y_mean"]
        return data_array

    def get_overlapping_timesteps(self, node) -> list:
        """
        Returns the common timesteps between self and node.
        """
        if (node.first_timestep > self.last_timestep) or (
            node.last_timestep < self.first_timestep
        ):
            return []
        return [
            max(self.first_timestep, node.first_timestep),
            min(self.last_timestep, node.last_timestep),
        ]
