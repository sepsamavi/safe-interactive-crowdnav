...
class MultimodalGenerativeCVAE(object):
    def __init__(
        ...
        if self.env.scenes is None:
            dt_container = self.env
        else:
            dt_container = self.env.scenes[0]
    ...
    def obtain_encoded_tensors(
        ...
        if (
            ...
            do = self.hyperparams["map_encoder"][self.node_type]["dropout"]  # dropout probability
