...
class SystemModel():
    ...
    def __init__(self,
        ...
        if "stacked_preds" in dynamics:
            self.stacked_preds = dynamics["stacked_preds"]
        else:
            self.stacked_preds = None
        # Setup for dynamics.
        ...
        self.term_cost_eqn = cost["term_cost_eqn"]
        logging.debug("cost equation: {:}".format(self.cost_eqn))
        self.Q_diag = cost["vars"]["Q_diag"]
        self.term_Q_diag = cost["vars"]["term_Q_diag"]
        self.R_diag = cost["vars"]["R_diag"]
    ...
    def setup_model(self):
        ...
        if self.stacked_preds is not None:
            self.f_func_nonlin = cs.Function('f', [self.x_sym, self.u_sym, self.stacked_preds], [self.x_kp1], ['x', 'u', 'stacked_preds'], ['f'])
        else:
    ...
    def setup_nonlinear(self):
        l_inputs = [self.x_sym, self.u_sym, self.Xr, self.Ur, self.Q_diag, self.R_diag]
        l_inputs_str = ['x', 'u', 'Xr', 'Ur', 'Q_diag', 'R_diag']
        l_outputs = [self.cost_eqn]
        l_outputs_str = ['l']
        self.cost_func = cs.Function('loss', l_inputs, l_outputs, l_inputs_str, l_outputs_str)
        term_l_inputs = [self.x_sym, self.Xr, self.term_Q_diag]
        term_l_inputs_str = ['x','Xr', 'term_Q_diag']
        term_l_outputs = [self.term_cost_eqn]
        term_l_outputs_str = ['l']
        self.term_cost_func = cs.Function('term_loss', term_l_inputs, term_l_outputs, term_l_inputs_str, term_l_outputs_str)
