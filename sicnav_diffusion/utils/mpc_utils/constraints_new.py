...
class BoundedConstraint(LinearConstraint):
    ...
    def __init__(self,
        ...
        self.row_names = ['lb{:}'.format(idx) for idx in range(self.lower_bounds.shape[0])] + ['ub{:}'.format(idx) for idx in range(self.upper_bounds.shape[0])]
        self.check_tolerance_shape()
        self.create_casadi_fn()